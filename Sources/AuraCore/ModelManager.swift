...
public final class ModelManager: ObservableObject {
    ...
    private func performLoad(
        ...
        let instance: AuraLocal
        switch model.purpose {
            case .text:
                instance = try await AuraLocal.text(model, onProgress: progress)
            case .vision:
                instance = try await AuraLocal.vision(model, onProgress: progress)
            case .visionSpecialized:
                instance = try await AuraLocal.specialized(model, onProgress: progress)
        }
